...
caller budget 默认值为 `max_output_chars=12000`、`max_pages=10`、`max_regions=4`，hard maximum 分别为 100000、100、4。budget 只适用于 schema 指定的 action；超限直接返回 `invalid_request`，不会 silent clamp 或开始 acquisition。`interact` 使用 server 默认 budget，不能携带 caller budget。

所有 Web Read instance 共用 process-wide admission：最多 2 个进行中的 `open/advance/interact/asset`，同一 state 的 processing 冲突立即拒绝；`read/find/release` 不占处理名额。每次新工作预留 32 MB temporary storage 和 768 MB RAM headroom；累计 retained Python state 上限 128 MB、单 state 32 MB、每 state 16 个 versions / 128 个 cursors。达到上限返回 `resource_exhausted` 和恢复提示，已有 versions、cursors 和 artifacts 保留，不自动 retry、refetch 或驱逐。caller 可 `release` 后显式重试；idle expiry 与 process exit 也清理相关资源。

受控 artifact 与 browser directory 的 temporary reservation 总额为 256 MB；最多保留 2 个 browser sessions，每个 profile / download / trace directory 预留 16 MB。browser RSS 512 MB、temporary usage 每 50 ms 检查；OCR worker RSS 检查阈值为 768 MB，process-tree RSS admission ceiling 为 3 GB。采样阈值不是操作系统实时 quota，可能有瞬时超量。Windows PDF/OCR native worker 另施加 Job Object 的 2 GB committed-memory hard limit；RSS 和 committed memory 不等价。保留 state 的主进程不执行 PDF native parsing/rasterization。

其他 server limits：HTTP acquisition 与 rendered DOM 各 2 MB、每次网页 operation 最多 8 张新 image / 累计 8 MB encoded bytes、browser response 10 MB、每 browser session 100 次 request、5 个 viewport scroll step、单 page、最多 2 个 renderer process 与 256 MiB V8 old-space；decoded image / PDF raster 最多 12000000 pixels，PDF crop output 最多 5 MB。默认 operation deadline 为 30 秒，HTTP、browser、image capture 与 OCR 共用该 deadline，最多 5 次 HTTP redirect。

URL、redirect、browser request 和网页 image URL 都检查 scheme、userinfo、DNS/IP public boundary；browser 不继承 credentials，不绕过登录、paywall、CAPTCHA 或访问控制。PDF 复用 native text，对 mixed/scanned regions 执行 CPU OCR；table / column / image reading order 不可靠时披露 `structure_incomplete`，caller 可用 crop 或原始 image 核对。数值依据及观测边界见 [#24 Windows resource evidence](docs/testing/issue24-windows-smoke.md)，这些保护值不是 latency / quality SLA。
