...
smoke 确认静态 response 为空壳时，JavaScript 生成的英文正文由 rendered DOM 抽取，`processing.path` 与 `browser_rendered` 披露实际路径。caller 使用响应中的 opaque `target_id` 和当前 `version` 执行 `expand`；响应只返回新增正文，`previous_version` 与新 `version` 对应实际变化。stdio 断开后 server lifecycle 关闭 browser context/process，stderr 为空。
...
browser context 不携带 host cookies 或 credentials，block service worker/download 和 interaction navigation；每个 browser request 继续通过 URL policy。边界包括 30 秒默认 deadline、100 次 browser request、2,000,000-byte rendered DOM、5 个 scroll step、16 个 immutable version、单 page、最多 2 个 renderer process 和 256 MiB V8 old-space。renderer process 与 V8 参数是 Chromium resource controls，并非 Windows Job Object 的整体 RSS hard cap。
